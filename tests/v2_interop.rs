...
fn an_independent_peer_interoperates_over_the_file_edge() {
    ...
    let mut peer = std::process::Command::new("python3")
        .arg(crate_root().join("tests/interop/peer.py"))
        .arg("--edge-dir")
        .arg(&edge)
        .arg("--schemas")
        .arg(crate_root().join("spec/schemas"))
    ...
    let mut send = |reference: &mut Transcript,
                    session: &Session,
                    from: &str,
                    to: &str,
                    kind: &str,
        ...
        session
            ...
            .expect("reference authors as a participant");
    ...
    let actual_digest =
        hacp::v2::canon::digest_canonical(&String::from_utf8_lossy(&artifact_bytes));
    let one_line =
        artifact_bytes.iter().filter(|b| **b == b'\n').count() == 1 && !artifact_bytes.is_empty();
    let size_ok = info["size"].as_u64() == Some(artifact_bytes.len() as u64);
    let checks = vec![
        Check {
            name: "digest-matches".into(),
            passed: actual_digest == claimed_digest,
            detail: "sha256 of the artifact bytes".into(),
        },
        Check {
            name: "file-has-one-line".into(),
            passed: one_line,
            detail: "newline count".into(),
        },
        Check {
            name: "size-matches".into(),
            passed: size_ok,
            detail: "manifest size".into(),
        },
    ];
    contract
        .submit(
    ...
    contract.apply_verification(&verification).unwrap();
    ...
    assert_eq!(
        peer_digest, digest,
        "§7.5 preimage agreement across implementations"
    );
    ...
    assert!(
        !rendered.contains("execute"),
        "EXECUTE never appeared on the wire (§7.5)"
    );
